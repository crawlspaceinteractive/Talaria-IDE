    ...
    try {
      ...
      if (verts && indices) {
        ...
        for (let i = 0; i < faceCount; i++) {
          ...
          faces[i * 9 + 0] = verts[i0 + 0]; faces[i * 9 + 1] = verts[i0 + 1]; faces[i * 9 + 2] = verts[i0 + 2];
          faces[i * 9 + 3] = verts[i1 + 0]; faces[i * 9 + 4] = verts[i1 + 1]; faces[i * 9 + 5] = verts[i1 + 2];
          faces[i * 9 + 6] = verts[i2 + 0]; faces[i * 9 + 7] = verts[i2 + 1]; faces[i * 9 + 8] = verts[i2 + 2];
        }

        // Compute bounds for topY / half extents
        ...
        const model = {
          meshData: { vertices: verts, normals, indices, colors },
          faces,
          faceCount,
          topY: maxY,
          halfW: (maxX - minX) * 0.5,
