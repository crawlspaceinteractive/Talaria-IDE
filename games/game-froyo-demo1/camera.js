...
const RAY_MAX_DIST  = 120;  // world units before we settle on the end point
const RAY_STEP      = 1.5;  // step size per iteration
const RAY_MUZZLE_UP = 0.25; // vertical offset from player center for ray origin
...
export function createCamera() {
  return {
    ...
    targetX: 0, targetY: 3.5, targetZ: -7,
    targetYaw: 0,
    targetPitch: 0,
    targetFovMul: 1.0,
...
export function castLookRay(camera, player, platforms) {
  ...
  const ox = px + dx * 0.40;  // muzzle start, slightly ahead
