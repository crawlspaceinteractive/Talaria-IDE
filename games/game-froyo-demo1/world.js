 ...
 */
import { JUMP_REACH_LUT, PLATFORM_HEIGHT_LUT } from "./luts.js";
import { weldBlockCorners, averageGroupCorners, getWeldedTopVerts, getAllWeldedVerts, magnetizeIslandEdges } from "./meshweld.js";
...
function buildBridge(rand, x0, y0, z0, x1, y1, z1, pal) {
  ...
  for (let i = 0; i <= count; i++) {
    ...
    planks.push({
      ...
      color: pal.top, side: pal.side, type: "bridge",
      blocks: [{
        wx, wy, wz,
...
// ─── Biome decorations ────────────────────────────────────────────────────────
function placeDecorations(rand, biome, ix, iy, iz, walkY, halfW, halfD, count) {
  const decors = [];
...
function sampleInSector(rand, sector, placedIslands, estHW, estHD, maxTries) {
  ...
  for (let t = 0; t < maxTries; t++) {
    ...
    sector.minR = Math.min(sector.minR * 1.05, maxR);
  }
...
export function generateWorld(seed = 1337) {
  ...
  const INNER_COUNT  = 5 + ((rand() * 2) | 0);  // 5–6
  const OUTER_COUNT  = 4 + ((rand() * 2) | 0);  // 4–5
  ...
  for (const sector of allSectors) {
    ...
    const worldBlocks = shape.blocks.map(b => ({
      ...b, wx: ix + b.ox, wy: iy + b.oy, wz: iz + b.oz,
    }));

    const walkY   = iy + shape.topY;
    const sy_phys = shape.topY;

    const WELD_THRESH = 0.8;
    ...
    const newDecors = placeDecorations(rand, pal.biome, ix, iy, iz, walkY, shape.halfW, shape.halfD, decorCount);
  ...
  // ── Magnetization pass ─────────────────────────────────────────────────
  {
    const childIslands = platforms.filter(p => p.type === "island" && p.blocks && p.blocks.length > 0);
