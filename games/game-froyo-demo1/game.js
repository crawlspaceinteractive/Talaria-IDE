 ...
 */
import { InputController, BTN_FLAGS } from "./input.js";
import { resolveBitwise, resolveDiscrete, STATE } from "./state.js";
import { generateWorld, stepMovingPlatforms } from "./world.js";
import { createCamera, updateCamera, castLookRay } from "./camera.js";
...
import { loadSave, writeSave, downloadFroyoFile } from "./persistence.js";
import { sceneDataToWorld } from "./mapgen-export.js";
import {
...
export class FroyoGame {
  ...
  _tickMenu() {
    ...
    const MENU_COUNT = 5; // Start, Settings, Load Map, Save, Reset
    // Vertical navigate
    ...
    drawCenterPanel(rd, [
      ...
      { text: sel(3) + "SAVE  FROYO",   scale: 1 },
      { text: sel(4) + "RESET PROGRESS",scale: 1 },
      { text: "",   scale: 1 },
  ...
  _renderScene(staticFrame, modes = null) {
    ...
    if (this._skyDomeMesh) {
      ...
      const skyDomeTris = buildMeshTris(
        ...
        "skyDome"                  // custom colorMode for purple-orange gradient
      );
      for (const t of skyDomeTris) tris.push(t);
    }
    if (this._skyRingMesh) {
      const SKY_RING_TINT = rgba(180, 80, 200);    // purple-ish inner ring
      const skyRingTris = buildMeshTris(
        ...
        projectVertex,
        this._skyRingScale,
        cam.yaw,                   // same parallax yaw
        "skyRing"
      );
      for (const t of skyRingTris) tris.push(t);
    }
