...
export class Raycaster {
  ...
  castAll(px, py, angle, numRays, fov) {
    ...
    const Q14    = 16384;  // Q1.14 scale

    const hitDist     = this._hitDist;
    const hitSide     = this._hitSide;
    const hitWallX    = this._hitWallX;
    const hitMapX     = this._hitMapX;
    const hitMapY     = this._hitMapY;
    const hitPrevMapX = this._hitPrevMapX;
    const hitPrevMapY = this._hitPrevMapY;

    // Build a thin accessor array — each element is an object whose fields
    // point back into the typed arrays.  Built once, reused every frame.
    if (!this._hitView || this._hitView.length !== numRays) {
      this._hitView = new Array(numRays);
      for (let i = 0; i < numRays; i++) {
        // Capture typed-array slot so the object shape is monomorphic.
        const ri = i;
        this._hitView[i] = {
          get dist()     { return hitDist[ri];     },
          get side()     { return hitSide[ri];     },
          get wallX()    { return hitWallX[ri];    },
          get mapX()     { return hitMapX[ri];     },
          get mapY()     { return hitMapY[ri];     },
          get prevMapX() { return hitPrevMapX[ri]; },
          get prevMapY() { return hitPrevMapY[ri]; },
        };
      }
    }
    ...
    for (let i = 0; i < numRays; i++) {
      ...
      // Q1.14 * float → still float (no fixed-multiply needed here).
      const cx = soaCos[i] / Q14;
      const cy = soaSin[i] / Q14;
      const wx = camCosF * cx - camSinF * cy;
      const wy = camSinF * cx + camCosF * cy;

      this._ddaIntoSlot(px, py, wx, wy, i);
    }

    return this._hitView;
  }

  // ── DDA — writes directly into the pre-allocated typed-array slots.
  _ddaIntoSlot(px, py, cos, sin, slot) {
    const map  = this.map;
    const mapX0 = Math.floor(px);
    const mapY0 = Math.floor(py);

    const deltaDistX = Math.abs(cos) < 1e-10 ? 1e30 : Math.abs(1 / cos);
    const deltaDistY = Math.abs(sin) < 1e-10 ? 1e30 : Math.abs(1 / sin);
  ...
  // ── Hitscan ray (center of screen) — still used by shootRay().
  shootRay(px, py, angle) {
    const cosF = CosineTable[radToBAM(angle)] / FP_ONE;
    const sinF = SineTable[radToBAM(angle)]   / FP_ONE;
    // Reuse slot 0 as a scratch register — caller ignores return value anyway.
    this._ddaIntoSlot(px, py, cosF, sinF, 0);
    const d = this._hitDist[0];
    if (d > 20) return null;
    return null; // enemy hit detection done by sprite distance check
  }
  ...
  hasLOS(px, py, ex, ey) {
    ...
    for (let i = 1; i < steps; i++) {
      const t = i / steps;
      if (this.map.isWall(px + dx * t, py + dy * t)) return false;
    }
    return true;
  }
  ...
  attachSticker(mapX, mapY, worldX, worldY, worldZ, tex_id, width = 0.25, height = 0.25, animated = false) {
    ...
    if (Math.abs(dx) > Math.abs(dy)) {
      ...
      t     = (worldY - mapY);          // 0..1 along Y extent of tile
      fromX = mapX + (dx > 0 ? 1 : -1); // neighbor in the X direction the sticker faces
      fromY = mapY;
    } else {
      // Y face (top or bottom wall boundary)
      t     = (worldX - mapX);          // 0..1 along X extent of tile
      fromX = mapX;
      fromY = mapY + (dy > 0 ? 1 : -1); // neighbor in the Y direction the sticker faces
    }
    ...
    const sticker = {
      ...
      animated,
      frame: 0,
      // fromX/fromY: the open-space tile this face is visible from.
      // Matches hit.prevMapX/prevMapY from the DDA, so _drawWallStickers
      // can reject rays that struck a different face of the same wall tile.
      fromX,
      fromY,
    };
