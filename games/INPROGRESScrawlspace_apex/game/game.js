...
export class FroyoGame {
  ...
  async _loadFroyoMesh() {
    // Player Model uploaded by user
    const GLB_URL = `${MODEL_BASE_URL}player_body_model.glb`;
    try {
      await threeReady();
      const mesh = await loadGLBMeshIfAvailable(GLB_URL, "Player body model");
      if (!mesh) return;
  ...
  _enterGameplay() {
    ...
    flashMessage(this.hud, "GET STOKED!", 90);
  ...
  _tickLoading() {
    ...
    drawText(rd, "LOADING GEOMETRY", bar1X, bar1Y - 9, rgba(150, 200, 255), 1);
  ...
  _tickMenu() {
    ...
    drawCenterPanel(rd, [
      { text: "crawlspace  ENGINE", scale: 2, color: rgba(255, 0, 0) },
      { text: "v1.2", scale: 1, color: rgba(255, 0, 0) },
      { text: "",   scale: 1 },
...
if (this._skyRingMesh) {
  const SKY_RING_TINT = rgba(180, 80, 200);
